"""
robot_description / ik_execute_suction_5axis.py
--------------------------------------------------
suction 그리퍼: 좌표 입력 -> IK 경로 -> PyBullet 미리보기(흔들림 없음)
...
실행:
  ros2 run robot_description ik_execute_suction_5axis --ros-args -p use_robot:=false
좌표:
...
URDF_NAME = 'robot_arm_5axis_suction.urdf'


class ExecNode(Node):
    def __init__(self):
        super().__init__('ik_execute_suction_5axis')
        ...
        self.declare_parameter('dxl_port', '/dev/ttyUSB1')
        ...
        self.get_logger().info('Ready (suction, fix_mode=%s, use_robot=%s).'
